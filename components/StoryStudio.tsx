...
const StoryStudio: React.FC<StoryStudioProps> = ({ character, allCharacters, onUpdate, onBack }) => {
  ...
  const handleSendMessage = async () => {
    ...
    const textForAPI = userInput;
    setUserInput('');
    setIsChatLoading(true);
    setChatError(null);
    try {
        const modelResponse = await chatWithCharacter(character, chatHistory, textForAPI);
        const newModelMessage: ChatMessage = { role: 'model', parts: [{ text: modelResponse.textResponse }], isGeneratingImage: !!modelResponse.imagePrompt };
 ...
 const handleGenerateCustomImage = async () => {
  ...
  const handleGetSuggestions = async () => {
    ...
    try {
        ...
        // For simplicity, we'll just apply the first suggestion.
        // A more complex UI could let the user choose between the 3.
                 ...
                 <div className="bg-gray-800/50 rounded-lg overflow-hidden border border-gray-700">
                    <div className="w-full aspect-[9/16] bg-gray-900 flex items-center justify-center">
                      {isImageLoading ? (
                        ...
                        <img src={latestImageUrl || `https://placehold.co/900x1600/1a1a1a/333333?text=${encodeURIComponent(character.name)}`} alt={character.name} className="w-full h-full object-cover" />
                      )}
                    </div>
                    <div className="p-4">
                        <h2 className="text-2xl font-bold">{character.name}</h2>
                        <p className="text-pink-400 font-semibold">{character.personality}, {character.occupation}</p>
                    </div>
                    <div className="p-4 border-t border-gray-700">
                        <button onClick={() => setIsGalleryModalOpen(true)} className="w-full bg-gray-700 text-white font-bold py-2.5 rounded-lg hover:bg-gray-600 transition-colors">Xem Album</button>
                    </div>
                 </div>
                          ...
                          {chatHistory.map((msg, index) => (
                             <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                 <div className={`max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-2xl ${msg.role === 'user' ? 'bg-pink-600 text-white rounded-br-none' : 'bg-gray-700 text-gray-200 rounded-bl-none'}`}>
                                      ...
                                      {msg.imageUrl && (<img src={msg.imageUrl} alt="Generated in chat" className="mt-2 rounded-lg max-w-full h-auto cursor-pointer" onClick={() => setIsGalleryModalOpen(true)}/>)}
                                 </div>
                             </div>
                          ))}
